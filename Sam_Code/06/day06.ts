...
readFile("06/input.txt", (err, data) => {
    ...
    console.log(`The number of unique answered questions per group in total is ${sum(questionsAnswered)}`);
    ...
    console.log(`The number of unique answered questions answered by everyone in each group in total is ${sum(allQuestionsAnswered)}`);
});
